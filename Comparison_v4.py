...
criteria = ['mse', 'matched_mae', 'matched_mae(m)', 'average_depth', 'est_depth', 'gt_depth', 'distance']
...
class ResultCalculator(Estimates):
    ...
    def matching_mae(self, scale=0.3, cuda=0):
        ...
        for (pred_ind, take, group, segment, sample) in tqdm(self.tags.itertuples(), total=self.tags.shape[0]):
            ...
            self.result.loc[pred_ind, (source, ['matched_mae', 'matched_mae(m)', 'average_depth', 'est_depth', 'gt_depth', 'distance'])] = [
...
class PropMergeResults(ProposedResults):
    def __init__(self, *args, **kwargs):
        super(PropMergeResults, self).__init__(*args, **kwargs)
    ...
    def resize_loss(self):
        ...
        for (pred_ind, take, group, segment, sample) in tqdm(self.tags.itertuples(), total=self.tags.shape[0]):
            ...
            img = np.squeeze(self.preds['SC_PRED'][pred_ind]) * self.depth[source][pred_ind]
...
class ResultProcess:
    ...
    def load_preds(self):
        for sub, path in self.subjects.items():
            ...
            if 'merge' in sub:
                self.subjects[sub] = PropMergeResults(name=sub, path=path,
                                                     gt=self.most_gt,
    ...
    def schedule(self):
        ...
        self.matching_mae()
        self.segment_mean()
        for item in ('mse', 'matched_mae', 'distance'):
