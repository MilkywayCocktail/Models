...
class TrainerVTS_V05c3:
    def __init__(self, img_encoder, img_decoder, pd_encoder,
                 lr, epochs, cuda,
        ...
        self.models = {'imgen': img_encoder.to(self.device),
                       ...
                       'pden': pd_encoder.to(self.device)
                       }
    ...
    def calculate_loss_s_latent(self, pd, c_img, i=None):

        s_z, s_mu, s_logvar, s_bbx = self.models['pden'](pd)
    ...
    def calculate_loss_s_bbx(self, pd, c_img, bbx, i=None):
    ...
    def train_teacher(self, autosave=False, notion=''):
        ...
        for epoch in range(self.epochs):
            ...
            for idx, (pd, img, bbx, index) in enumerate(self.train_loader, 0):
            ...
            for idx, (pd, img, bbx, index) in enumerate(self.valid_loader, 0):
                img = img.to(torch.float32).to(self.device)
                with torch.no_grad():
                    PREDS = self.calculate_loss_t(img)
                for key in EPOCH_LOSS.keys():
    ...
    def train_student(self, autosave=False, notion=''):
        ...
        """
        optimizer = self.optimizer([{'params': self.models['pden'].parameters()}], lr=self.lr)
        ...
        for epoch in range(self.epochs):
            ...
            for idx, (pd, img, bbx, index) in enumerate(self.train_loader, 0):
                ...
                if self.mode == 'latent':
                    PREDS = self.calculate_loss_s_latent(pd, img)
                elif self.mode == 'bbx':
                    PREDS = self.calculate_loss_s_bbx(pd, img, bbx)
                optimizer.zero_grad()
                self.temp_loss['LOSS'].backward()
                optimizer.step()
            ...
            for idx, (pd, img, bbx, index) in enumerate(self.valid_loader, 0):
                ...
                if autosave:
                    ...
                    if val_loss < best_val_loss:
                        ...
                        torch.save(
                            {"pden": self.models['pden'].state_dict()},
                            f"{save_path}{notion}_{self.models['pden']}_{self.mode}_best.pth",
                        )
    ...
    def test_teacher(self, loader='test'):
        ...
        for idx, (pd, img, bbx, index) in enumerate(loader, 0):
            img = img.to(torch.float32).to(self.device)
            with torch.no_grad():
    ...
    def test_student(self, loader='test'):
        ...
        for idx, (pd, img, bbx, index) in enumerate(loader, 0):
            ...
            with torch.no_grad():
                for sample in range(loader.batch_size):
                    ind_ = index[sample][np.newaxis, ...]
                    pd_ = pd[sample][np.newaxis, ...]
                    img_ = img[sample][np.newaxis, ...]
                    bbx_ = bbx[sample][np.newaxis, ...]
                    if self.mode == 'latent':
                        PREDS = self.calculate_loss_s_latent(pd_, img_, ind_)
                    elif self.mode == 'bbx':
                        PREDS = self.calculate_loss_s_bbx(pd_, img_, bbx_, ind_)
