...
class TrainerVTS_V05c4:
    ...
    def calculate_loss_s_latent(self, csi, pd, c_img, i=None):

        s_z, s_mu, s_logvar, s_bbx = self.models['csien'](csi, pd)
    ...
    def calculate_loss_s_bbx(self, csi, pd, c_img, bbx, i=None):
    ...
    def train_student(self, autosave=False, notion=''):
        ...
        for epoch in range(self.epochs):
            ...
            for idx, (csi, img, pd, bbx, index) in enumerate(self.train_loader, 0):
                ...
                if self.mode == 'latent':
                    PREDS = self.calculate_loss_s_latent(csi, pd, img)
                elif self.mode == 'bbx':
                    PREDS = self.calculate_loss_s_bbx(csi, pd, img, bbx)
                optimizer.zero_grad()
                self.temp_loss['LOSS'].backward()
                optimizer.step()
        ...
        for idx, (csi, img, pd, bbx, index) in enumerate(loader, 0):
            ...
            with torch.no_grad():
                for sample in range(loader.batch_size):
                    ...
                    if self.mode == 'latent':
                        PREDS = self.calculate_loss_s_latent(csi_, pd_, img_, ind_)
                    elif self.mode == 'bbx':
                        PREDS = self.calculate_loss_s_bbx(csi_, pd_, img_, bbx_, ind_)
